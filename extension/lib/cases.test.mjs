...
import {
  WOODEN_TIERS, WOODEN_PP, CASE_ODDS, GEAR_CODES, expectedQty, woodenCaseValue, caseScrapValue, caseMarketValue,
  caseVerdict, tripCost, regionsAwayFromText, casesSummary,
} from './cases.mjs';
...
describe('the wooden case rules', () => {
  it('carries the four tiers of the patch notes, 20 resources, 100% in all', () => {
    ...
    for (const code of WOODEN_TIERS.flatMap((t) => t.items)) expect(WOODEN_PP[code]).toBeGreaterThan(0);
  });
  ...
  it('expects the budget divided by the production points, floored, never below one', () => {
    ...
    expect(expectedQty(40, 'round')).toBeCloseTo((20 * 1 + 20 * 1 + 20 * 2 + 1 * 2) / 61, 6);   // 20..39 -> 1 (0 or 1 lifted to 1), 40..59 -> 1, 60..80 -> 2
  });
  ...
  it('opens for 6.12 gold at the pinned bids (round rule 6.52 as the band)', () => {
    ...
    expect(v.tiers.map((t) => t.tier)).toEqual(['epic', 'rare', 'uncommon', 'common']);
  });
...
describe('the battle cases', () => {
  ...
  it('lists the 36 gear codes, six a rarity, one weapon each', () => {
    expect(Object.keys(GEAR_CODES)).toEqual(['common', 'uncommon', 'rare', 'epic', 'legendary', 'mythic']);
  ...
  it('values an open at the average item price, weighted by the audited rarity odds and the 30% weapon share', () => {
    const tier = { common: 1, uncommon: 2, rare: 3, epic: 4, legendary: 5, mythic: 6 };
    const avgOf = (code) => { for (const [r, g] of Object.entries(GEAR_CODES)) if (g.weapon === code || g.gear.includes(code)) return 100 * tier[r]; return null; };
    const v = caseMarketValue('case1', avgOf);
    expect(v.value).toBeCloseTo(147.09, 1);
    expect(v.complete).toBe(true);
    expect(caseMarketValue('case2', avgOf).value).toBeCloseTo(0.503745 * 200 + 0.324203 * 300 + 0.143002 * 400 + 0.025305 * 500 + 0.003745 * 600, 1);
  });

  it('averages what it has inside a rarity and is incomplete when a whole rarity is unpriced', () => {
    const avgOf = (code) => (code === 'jet' ? null : code === 'knife' ? 2 : /1$/.test(code) ? 4 : /6$/.test(code) ? 900 : 10);
    const v = caseMarketValue('case1', avgOf);
    expect(v.complete).toBe(true);   // mythic still has its five gear codes
    const none = caseMarketValue('case1', (code) => (/6$|jet/.test(code) ? null : 5));
    expect(none.complete).toBe(false);
    expect(none.missing).toEqual(['mythic']);
...
describe('caseVerdict', () => {
  it('says sell, open or even with the 10% rule', () => {
    expect(caseVerdict({ bid: 21.701, openValue: 14.92 })).toMatchObject({ verdict: 'sell' });
    expect(caseVerdict({ bid: 21.701, openValue: 14.92 }).ratio).toBeCloseTo(21.701 / 14.92, 6);
    expect(caseVerdict({ bid: 3.439, openValue: 3.2676 })).toMatchObject({ verdict: 'even' });
    expect(caseVerdict({ bid: 5, openValue: 6.5 })).toMatchObject({ verdict: 'open' });
    expect(caseVerdict({ bid: 5.5, openValue: 5 })).toMatchObject({ verdict: 'sell' });
  });
...
describe('tripCost', () => {
  ...
  it('is free on the bar and a loss past the breakeven', () => {
    ...
    expect(tripCost({ hops: 0, oilAsk: 0.242, value: 7.55 }).netOneWay).toBeCloseTo(7.55, 6);
...
describe('casesSummary', () => {
  const books = Object.fromEntries(Object.entries(BIDS).map(([code, bid]) => [code, { bid, ask: bid * 1.01, bidQty: 100, askQty: 50 }]));
  ...
  it('gives one row per case with bid, open values and a verdict, plus the oil ask', () => {
    ...
    const c1 = s.rows[1];
    expect(c1.openScrap).toBeCloseTo(3.26757, 4);
    expect(c1.openMarket).toBeCloseTo(10, 6);
    expect(c1.openValue).toBeCloseTo(10, 6);        // the better of the two ways to cash an open
    expect(c1.verdict).toBe('open');
  ...
  it('falls back to the scrap floor when the average prices are not there', () => {
    ...
    expect(s.rows[1].openValue).toBeCloseTo(3.26757, 4);
...
describe('caseMarketValue and a zero average', () => {
  it('never averages a zero in: a thin code pulls nothing toward zero', () => {
    ...
    expect(v.value).toBeCloseTo(10, 6);   // every priced code is 10, the zeros are ignored
    expect(v.complete).toBe(true);
