...
import { BOOK_URL, KEYLESS_LIMIT, ApiError, fetchBook, fetchSales, salesUrl, holdUntil } from './api.mjs';
...
describe('fetchSales row hygiene', () => {
  ...
  it('skips a fill whose price is not a number', async () => {
    const { fetchImpl } = pagedFake([{ items: [fill(1, 'abc'), fill(2, 5)], next: null }]);
    const r = await fetchSales(fetchImpl, 'k', 'jet', { hours: 72, now: NOW });
...
describe('timeouts and network failures', () => {
  const hanging = (url, opts) => new Promise((_, reject) => opts.signal.addEventListener('abort', () => reject(new DOMException('aborted', 'AbortError'))));

  it('gives up on a call that does not answer in time', async () => {
    await expect(fetchBook(hanging, 'k', { timeoutMs: 20 })).rejects.toMatchObject({ code: 'timeout' });
    await expect(fetchSales(hanging, 'k', 'jet', { now: NOW, timeoutMs: 20 })).rejects.toMatchObject({ code: 'timeout' });
  });
  ...
  it('reports an unreachable API as a network error, not an internal one', async () => {
    ...
    await expect(fetchBook(down, 'k')).rejects.toMatchObject({ code: 'network' });
  });
});
...
describe('holdUntil', () => {
  it('holds for the reset the API asked for, within sane bounds', () => {
    ...
    expect(holdUntil(9999, 1000)).toBe(1000 + 120_000);   // never wait forever on a bad header
...
import { booksUrl, batchUrls, fetchBooks, fetchEquipmentAvg, BATCH_URL_BUDGET } from './api.mjs';

const top = (code, bid, ask) => ({
  buyOrders: bid == null ? [] : [{ _id: 'b', user: 'u', itemCode: code, quantity: 40, price: bid, type: 'buy' }],
  sellOrders: ask == null ? [] : [{ _id: 's', user: 'u', itemCode: code, quantity: 7, price: ask, type: 'sell' }],
});

describe('batchUrls', () => {
  it('builds one tRPC batch URL with positional inputs', () => {
    const urls = batchUrls('gameStat.getEquipmentAvgByCode', [{ itemCode: 'jet' }, { itemCode: 'knife' }]);
    expect(urls).toHaveLength(1);
    expect(urls[0].url).toBe('https://api2.warera.io/trpc/gameStat.getEquipmentAvgByCode,gameStat.getEquipmentAvgByCode?batch=1&input='
      + encodeURIComponent(JSON.stringify({ 0: { itemCode: 'jet' }, 1: { itemCode: 'knife' } })));
    expect(urls[0].count).toBe(2);
  });

  it('splits by URL length, never by a fixed count, and keeps the order', () => {
    const inputs = Array.from({ length: 36 }, (_, i) => ({ itemCode: `helmet${i}` }));
    const urls = batchUrls('gameStat.getEquipmentAvgByCode', inputs);
    expect(urls.length).toBeGreaterThan(1);
    for (const u of urls) expect(u.url.length).toBeLessThanOrEqual(BATCH_URL_BUDGET);
...
describe('fetchBooks', () => {
  it('reads every code in one call and flattens the best bid and ask per code', async () => {
    const { fetchImpl, calls } = fake({ body: { result: { data: { woodenCase: top('woodenCase', 7.5, 7.647), oil: top('oil', 0.226, 0.242), scraps: top('scraps', null, 0.223) } } } });
    const r = await fetchBooks(fetchImpl, 'my-key', ['woodenCase', 'oil', 'scraps']);
    expect(calls).toHaveLength(1);
    expect(calls[0].url).toBe(booksUrl(['woodenCase', 'oil', 'scraps']));
    expect(calls[0].opts.headers['x-api-key']).toBe('my-key');
    expect(r.books.woodenCase).toEqual({ bid: 7.5, ask: 7.647, bidQty: 40, askQty: 7 });
    expect(r.books.scraps).toEqual({ bid: null, ask: 0.223, bidQty: 0, askQty: 7 });
  ...
  it('falls back to the documented getTopOrders, batched, when the multi-code procedure is gone', async () => {
    ...
    const fetchImpl = async (url, opts) => {
      calls.push({ url, opts });
      const headers = new Headers({ 'ratelimit-limit': '500', 'ratelimit-remaining': '400', 'ratelimit-reset': '60' });
      if (url.includes('getTopOrdersPerItemCode')) return { ok: false, status: 404, headers, json: async () => ({ error: { message: 'No procedure found on path' } }) };
      return { ok: true, status: 200, headers, json: async () => [{ result: { data: top('oil', 0.226, 0.242) } }, { result: { data: top('scraps', 0.221, 0.223) } }] };
    };
    const r = await fetchBooks(fetchImpl, 'k', ['oil', 'scraps']);
    expect(calls).toHaveLength(2);
    expect(calls[1].url).toContain('tradingOrder.getTopOrders,tradingOrder.getTopOrders?batch=1');
  ...
  it('refuses without a key and rejects a keyless-bucket answer', async () => {
    const { fetchImpl, calls } = fake();
    await expect(fetchBooks(fetchImpl, '', ['oil'])).rejects.toMatchObject({ code: 'no-key' });
    expect(calls).toHaveLength(0);
    const keyless = fake({ limit: KEYLESS_LIMIT, body: { result: { data: { oil: top('oil', 1, 2) } } } });
    await expect(fetchBooks(keyless.fetchImpl, 'wrong', ['oil'])).rejects.toMatchObject({ code: 'key-rejected' });
  });
});
...
describe('fetchEquipmentAvg', () => {
  it('reads the average price per code through batches and keys the numbers by code', async () => {
    ...
    const fetchImpl = async (url, opts) => {
      ...
      const out = Object.values(input).map((i) => (i.itemCode === 'broken' ? { error: { message: 'nope' } } : { result: { data: i.itemCode.length * 10 } }));
      return { ok: true, status: 200, headers: new Headers({ 'ratelimit-limit': '500', 'ratelimit-remaining': '400', 'ratelimit-reset': '60' }), json: async () => out };
    };
    const r = await fetchEquipmentAvg(fetchImpl, 'k', ['jet', 'knife', 'broken', 'helmet3']);
    expect(calls).toHaveLength(1);
  ...
  it('refuses without a key', async () => {
    const { fetchImpl, calls } = fake();
    await expect(fetchEquipmentAvg(fetchImpl, ' ', ['jet'])).rejects.toMatchObject({ code: 'no-key' });
    expect(calls).toHaveLength(0);
  });
});
...
describe('fetchEquipmentAvg treats zero as missing', () => {
  it('drops a 0 and names it, keeps real prices', async () => {
    const fetchImpl = async (url) => {
      const input = JSON.parse(new URL(url).searchParams.get('input'));
      const out = Object.values(input).map((i) => ({ result: { data: i.itemCode === 'helmet6' ? 0 : 12.5 } }));
      return { ok: true, status: 200, headers: new Headers({ 'ratelimit-limit': '500', 'ratelimit-remaining': '400', 'ratelimit-reset': '60' }), json: async () => out };
    };
